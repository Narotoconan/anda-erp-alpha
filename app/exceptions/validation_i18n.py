...
_ERROR_TYPE_MAP: dict[str, str | Callable[[dict], str]] = {
    ...
    "too_small":                lambda ctx: (
        f"值不能小于 {ctx['ge']}"   if ctx.get("ge") is not None else
        f"值必须大于 {ctx['gt']}"   if ctx.get("gt") is not None else
        "值过小，超出允许范围"
    ),
    # 修复: lt(严格小于) 与 le(小于等于) 语义不同，必须区分表达
    "too_big":                  lambda ctx: (
        f"值不能大于 {ctx['le']}"   if ctx.get("le") is not None else
        f"值必须小于 {ctx['lt']}"   if ctx.get("lt") is not None else
        "值过大，超出允许范围"
    ),
